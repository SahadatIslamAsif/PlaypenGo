...
- Email via Nodemailer + SMTP. Rendered with `react-email` — inline styles only, Tailwind classes do not survive email clients. Signup's own confirmation email is a separate path — Supabase GoTrue's own send, on a hand-inlined HTML template (`supabase/templates/confirmation.html`), not this pipeline.
...
- **One email per person per day, maximum.** Everything folds into the 7:00 PM digest. Nothing sends when all sections are empty.
- Nothing from an AI parse is written to the database without passing through a human review screen. Every extracted field stays editable. **Exception: syllabus import** — see ARCHITECTURE.md §5.2 for why it commits straight through, and what that makes load-bearing instead.
